...
export default class ServiçosUsuário {
  ...
  static async verificarCadastroCompleto(usuário: Usuário) {
    switch(usuário.perfil) {
      case Perfil.JORNALISTA:
        const jornalista = await Jornalista.findOne({ where: { usuário: usuário.cpf },
          relations: ["usuário"] });
        if (!jornalista) return false;
        return true;
      case Perfil.EDITOR_JORNAL:
        const editor_jornal = await EditorJornal.findOne({ where: { usuário: usuário.cpf },
          relations: ["usuário"] });
        if (!editor_jornal) return false;
        return true;
      default: return;
    }
  };
  ...
  static async alterarUsuário(request, response) {
    try {
      ...
      const usuario_info = { nome: usuário.nome, perfil: usuário.perfil, email: usuário.email,
  ...
  static async buscarQuestãoSegurança(request, response) {
    ...
    } catch (error) { return response.status(500).json({ erro: "Erro BD: buscarQuestãoSegurança" }); }
  };
