...
export default function MenuLateral({ children }) {
  ...
  const opçõesJornalista = [
    { label: "Página Inicial", command: () => navegar("/página-inicial") },
    { label: "Menu", items: [
      { label: "Cadastrar Usuário", command: () => navegar("/atualizar-usuario"),
        disabled: usuárioLogado.status !== "ativo"},
      { label: "Cadastrar Jornalista", command: () => navegar("/cadastrar-jornalista")},
  ...
  const opçõesEditor = [
    ...
    { label: "Menu", items: [
      { label: "Cadastrar Usuário", command: () => navegar("/atualizar-usuario"),
        disabled: usuárioLogado.status !== "ativo"},
      { label: "Cadastrar Editor", command: () => navegar("/cadastrar-editor")},
      { label: "Sair do Sistema", command: () => sairSistema()}
    ]},
  ];
  ...
  function opçõesMenu() {
    switch (usuárioLogado.perfil) {
      case "jornalista": return opçõesJornalista;
      case "editor": return opçõesEditor;
      default: return;
    }
  };
