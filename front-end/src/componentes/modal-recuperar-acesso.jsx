...
import { TAMANHOS, estilizarBotão, estilizarDivCampo, estilizarLabel, estilizarModal, estilizarPasswordInput, estilizarPasswordTextInputBorder } from "../../utilitários/estilos";

export default function ModalRecuperarAcesso() {
  const referênciaToast = useRef(null);
  const { cpfVerificado, setCpfVerificado, novaSenha, setNovaSenha, tokenRecuperação, setTokenRecuperação } = useContext(ContextoUsuário);
  const [dados, setDados] = useState({ senha: novaSenha?.senha || "", confirmação: novaSenha?.confirmação || "" });
    ...
    <div className={estilizarModal()}>
      ...
      <div className={estilizarDivCampo()}>
        ...
        <Password name="confirmação" inputClassName={estilizarPasswordTextInputBorder()} toggleMask
          className={estilizarPasswordInput(erros.senha || erros.confirmação)}
